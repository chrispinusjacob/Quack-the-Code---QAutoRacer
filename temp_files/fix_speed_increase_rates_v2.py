...
print("Fixing difficulty speed increase rates (version 2)...")
...
if "speed_increase_rate" in difficulty_settings_content:
    ...
    # Modify the difficulty settings to have more noticeable speed increase rates
    ...
    if "\"easy\": {" in difficulty_settings_content:
        ...
        # Make easy mode increase speed very slowly but still noticeable
        ...
        print("EASY difficulty updated with more noticeable speed increase rate")
    ...
    if "\"medium\": {" in difficulty_settings_content:
        ...
        # Make medium mode have moderate speed increase
        difficulty_settings_content = difficulty_settings_content.replace(
        ...
        print("MEDIUM difficulty updated with more noticeable speed increase rate")
    ...
    if "\"hard\": {" in difficulty_settings_content:
        ...
        difficulty_settings_content = difficulty_settings_content.replace(
            "\"speed_increase_rate\": 0.0003,",
            "\"speed_increase_rate\": 0.0006,"
        )
        difficulty_settings_content = difficulty_settings_content.replace(
            "\"spawn_rate_increase\": 0.0002,",
            "\"spawn_rate_increase\": 0.0003,"
        )
        
        print("HARD difficulty updated with more noticeable speed increase rate")
...
# Update the difficulty manager to make speed increases more noticeable
print("\nUpdating difficulty manager to make speed increases more noticeable...")

with open("difficulty_manager.py", "r") as f:
    difficulty_manager_content = f.read()

# Modify the update method to make speed increases more noticeable
if "def update(self, dt):" in difficulty_manager_content:
    # Make the difficulty factor increase more quickly
    difficulty_manager_content = difficulty_manager_content.replace(
        "difficulty_factor = 1.0 + math.log(1 + self.game_time / 60)",
        "difficulty_factor = 1.0 + math.log(1 + self.game_time / 30)"
    )
    
    # Increase the multiplier for speed increases
    difficulty_manager_content = difficulty_manager_content.replace(
        "(self.speed_increase_rate * dt * 60 * difficulty_factor)",
        "(self.speed_increase_rate * dt * 120 * difficulty_factor)"
    )
    ...
    print("Difficulty manager updated to make speed increases more noticeable")
...
print("\\nDifficulty Speed Increase Test (v2)\\n" + "="*35)
...
class MockDifficultyManager:
    ...
    def update(self, dt):
        ...
        self.scroll_speed = min(
            ...
            self.initial_scroll_speed + (self.speed_increase_rate * dt * 120 * difficulty_factor)
        )
        
        self.enemy_speed = min(
            self.max_enemy_speed,
            self.initial_enemy_speed + (self.speed_increase_rate * dt * 120 * difficulty_factor)
        )
...
with open("verify_speed_increase_v2.py", "w") as f:
    f.write(test_content)

print("Created verify_speed_increase_v2.py to test updated difficulty settings")
...
print("\nSpeed increase rate fixes (v2) complete!")
print("Run verify_speed_increase_v2.py to check the updated difficulty settings")
